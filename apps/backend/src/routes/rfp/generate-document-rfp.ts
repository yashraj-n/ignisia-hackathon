  ...
  fastify.post<{ Params: { id: string }; Body: { choices?: { itemIndex: number; selectedOptionIndex: number }[] } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        fastify.log.info({ rfpId: id }, "Generating markdown content...");
        ...
        if (!markdownContent || markdownContent.trim().length < 10) {
          throw new Error("AI generated an empty or too-short document summary.");
        }

        // Convert markdown to PDF
        fastify.log.info({ rfpId: id }, "Converting Markdown to PDF via md-to-pdf...");
        ...
        try {
          ...
          if (!pdfResult?.content) {
            throw new Error("PDF conversion returned empty content (check Puppeteer installation).");
          }
          pdfBuffer = pdfResult.content;
        } catch (pdfError: any) {
          fastify.log.error({ pdfError }, "PDF conversion failed");
          throw new Error(`PDF Conversion Error: ${pdfError.message}`);
        }

        fastify.log.info({ rfpId: id }, "Uploading PDF");
        ...
        fastify.log.error({ error, rfpId: id }, "Document generation route fatal error");
        await db.rFP.update({
          where: { id },
          data: { status: "failed" },
        });

        throw fastify.httpErrors.internalServerError(error.message || "Document generation failed");
      }
    }
  );
}
